...
def save_frame_info_to_excel(camera_id, camera_name, frame, timestamp, dwell_time, object_id, checkout_area):
    ...
    frame_data = pd.DataFrame([[camera_id, camera_name, frame, timestamp, dwell_time, object_id, checkout_area]],
                              columns=columns)
...
def save_ssco_info_to_excel(camera_id, camera_name, frame, startTime, endTime, dwell_time, object_id, checkout_area):
    # Create a DataFrame with the frame information
    columns = ['Camera ID', 'Camera Name', 'Frame', 'StartTime', 'EndTime', 'Dwell Time (sec)', 'UniqueId', 'Area']
    frame_data = pd.DataFrame(
        [[camera_id, camera_name, frame, startTime, endTime, dwell_time, object_id, checkout_area]],
        columns=columns)

    # Check if the Excel file already exists
    excel_path = '/Users/mukeshnaidu/MukeshGit/output/ssco_info.xlsx'
    file_exists = os.path.isfile(excel_path)
